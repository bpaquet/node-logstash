...
BaseComponent.prototype.merge_config = function(config) {
  ...
  if (config.afterLoadConfigHook) {
    this.afterLoadConfigHooks.push(config.afterLoadConfigHook);
  }
}
...
BaseComponent.prototype.runAfterLoadConfigHooks = function(callback) {
  if (this.afterLoadConfigHooks.length == 0) {
    return this.afterLoadConfig(callback);
  }
  var hook = this.afterLoadConfigHooks.shift();
  hook.call(this, function() {
    this.runAfterLoadConfigHooks(callback);
  }.bind(this));
}
