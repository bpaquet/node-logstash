...
function MonitoredFile(filename, options) {
  ...
  this.logger = options.logger || new log4node.Log4Node('warning');
  this.logger.setPrefix('[FileMonitoring][' + path.basename(this.filename) + '] ');
    ...
    fs.exists(dir, function(exists) {
      if (exists) {
        try {
          ...
          this.logger.error('Unable to monitor ', dir, ':', err);
