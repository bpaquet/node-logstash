...
MonitoredFile.prototype.watch = function(callback) {
  try {
    ...
    var setup_dir_watcher = function() {
    ...
    var setup_dir_watcher_if_exists = function() {
      fs.exists(this.dir, function(dir_exists) {
        if (dir_exists) {
          setup_dir_watcher();
        } else {
          logger.warning('Dir ' + this.dir + ' does not exists, waiting for its creation');
          setTimeout(setup_dir_watcher_if_exists, 1000);
        }
      }.bind(this));
    }.bind(this);
    setup_dir_watcher_if_exists();
  }
  catch(err) {
