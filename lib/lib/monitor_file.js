      ...
      if (this.fdTailer) {
        ...
        this.restart(0, function(err) {
          ...
          if (this.current_ino === old_ino) {
            ...
            logger.info('Have reopen the same file, closing old fd now');
            close_func();
          }
        }.bind(this));
      }
      ...
      if (exists) {
        ...
        fs.stat(this.filename, function(err, stats) {
          ...
          if (stats.isFIFO()) {
            ...
            // nodeJS open.fs on a FIFO pipe callback is called only when some data arrive
            // we can wait to inform called init is done
            callback();
          }
