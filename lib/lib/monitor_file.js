...
MonitoredFile.prototype.start = function(start_index) {
  ...
  this.on('renamed', function() {
    if (this.fdTailer) {
      ...
      this.oldFdTailer = this.fdTailer;
      logger.debug('Closing file', this.oldFdTailer.fd, 'postponed', this.wait_delay_after_renaming);
      setTimeout(function() {
        logger.debug('Now closing postponed', this.oldFdTailer.fd);
        this.oldFdTailer.close();
        this.oldFdTailer = undefined;
...
function FdTailer(fd, current_index, options, event_target) {
  this.fd = fd;
...
FdTailer.prototype.close = function() {
  logger.info('Closing file', this.fd);
  fs.close(this.fd, function(err) {
    this.event_target.emit('closed');
    if (err) {
      return this.event_target.emit('error', err);
    }
  }.bind(this));
}

FdTailer.prototype.read = function() {
  if (!this.fd) {
    return;
  }
  if (this.read_in_progress) {
    ...
    logger.debug('Launch reading on', this.fd, ', current_index', this.current_index);
...
FdTailer.prototype.handle = function(length) {
  ...
  while (true) {
    ...
    if (index > 0) {
      this.event_target.emit('data', this.to_be_processed.slice(0, index));
    }
