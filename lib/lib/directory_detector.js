...
DirectoryDetector.prototype.start = function(directory) {
  ...
  fs.exists(this.directory, function(exists) {
    if (exists) {
      ...
      this.parent_detector.on('error', function(err) {
        this.emit('error', err);
      }.bind(this));
      this.parent_detector.on('exists', function() {
        ...
        try {
          ...
          logger.error('Unable to watch directory', this.parent);
          logger.error(e);
          this.emit('error', e);
        }
        fs.exists(this.directory, function(exists) {
      ...
      this.parent_detector.start(this.parent);
    }
  }.bind(this));
};
