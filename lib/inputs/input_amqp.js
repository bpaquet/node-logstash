...
function InputAmqp() {
  ...
  this.mergeConfig({
    ...
    host_field: 'host',
    port_field: 'port',
    optional_params: ['exchange_name', 'queue_name', 'topic', 'durable', 'retry_delay', 'heartbeat', 'type', 'username', 'password', 'vhost'],
    default_values: {
...
InputAmqp.prototype.consume = function(channel, queue) {
...
InputAmqp.prototype.start = function(callback) {
  ...
  this.connected_callback = function(channel) {
    if (this.queue_name) {
      logger.info('Start AMQP listener to', this.amqp_url, 'queue', this.queue_name);
      channel.assertQueue(this.queue_name, {}, function(err, queue) {
        if (err) {
          logger.error('Unable to assert queue: ' + this.queue_name, err);
        }
