...
function InputRedis() {
  ...
  this.config = {
    ...
    optional_params: ['error_buffer_delay', 'pattern_channel', 'retry', 'unserializer'],
    default_values: {
...
InputRedis.prototype.afterLoadConfig = function(callback) {
  ...
  this.error_buffer = error_buffer.create('input Redis to ' + this.host + ':' + this.port, this.error_buffer_delay, this);
