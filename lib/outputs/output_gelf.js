...
function OutputGelf() {
  ...
  this.config = {
    ...
    host_field: 'host',
    port_field: 'port',
    optional_params: ['error_buffer_delay', 'version', 'message', 'facility', 'level', 'additional_fields'],
    default_values: {
...
OutputGelf.prototype.process = function(data) {
  ...
  if (typeof data[this.additional_fields] != "undefined") {
    for (key in data[this.additional_fields]) {
      m["_" + key] = data[this.additional_fields][key];
    }
  }
