...
OutputSqs.prototype.start = function(callback) {
  logger.info('Creating AWS SQS Output to', this.aws_host);
  callback();
};

OutputSqs.prototype.process = function(data) {
  this.aws_client.call('SendMessage', {
    ...
    if (err) {
      ...
      if (!result.SendMessageResult || ! result.SendMessageResult.MessageId) {
...
OutputSqs.prototype.close = function(callback) {
  logger.info('Closing AWS SQS Output to', this.aws_host);
  callback();
};
