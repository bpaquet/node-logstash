...
function OutputElasticSearch() {
  ...
  this.mergeConfig({
    name: 'Elastic search',
    optional_params: ['bulk_limit', 'bulk_timeout', 'index_prefix', 'data_type','index'],
    default_values: {
  ...
  console.log ("ES index: " + this.index)
}
...
OutputElasticSearch.prototype.sendBulk = function() {
  var path = '/' + this.index + '/_bulk'
  if (!this.index) 
    path = elastic_search_helper.computePath(this.index_prefix, this.data_type) + '/_bulk';
...
OutputElasticSearch.prototype.process = function(data) {
  if (this.bulk_limit) {
    ...
    var path = this.index ? ('/' + this.index + '/' + this.data_type+'/') : elastic_search_helper.computePath(this.index_prefix, this.data_type)
    //if (!this.index) 
    //console.log (path)
    this.postData(path, JSON.stringify(data));
  }
};
