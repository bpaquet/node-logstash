...
AbstractTcp.prototype.findConnection = function(callback) {
  ...
  if (this.ssl) {
    connection = tls.connect(ssl_helper.merge_options(this, {host: this.host, port: this.port}), listener);
  }
  else if (this.unix_socket) {
    connection = net.createConnection({path: this.unix_socket}, listener);
  }
  else {
    connection = net.createConnection({host: this.host, port: this.port}, listener);
  }
