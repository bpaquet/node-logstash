...
AbstractZeroMQ.prototype.startAbstract = function(callback) {
  ...
  (Array.isArray(this.address) ? this.address : this.address.split(',')).forEach(function(address) {
...
AbstractZeroMQ.prototype.check = function() {
  if (this.on_alarm) {
    if (this.zmq_threshold_down && this.socket._outgoing.length < this.zmq_threshold_down) {
      logger.warning('Zmq socket end of alarm', this.address, 'current queue size', this.socket._outgoing.length);
      this.on_alarm = false;
      this.emit('alarm', false, this.address);
    }
    else {
      logger.info('Still in alarm : queue size : ', this.socket._outgoing.length);
    }
  }
};
...
AbstractZeroMQ.prototype.process = function(data) {
  this.formatPayload(data, function(message) {
    ...
    if (!this.on_alarm && this.zmq_threshold_down && this.zmq_threshold_up && this.socket._outgoing.length > this.zmq_threshold_up) {
      logger.warning('Zmq socket in alarm', this.address, 'current queue size', this.socket._outgoing.length);
      this.on_alarm = true;
...
AbstractZeroMQ.prototype.close = function(callback) {
  logger.info('Closing output to zeromq', this.address);
