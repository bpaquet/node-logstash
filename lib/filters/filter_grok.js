...
function FilterGrok() {
  ...
  this.mergeConfig(this.tags_fields_config());
  this.mergeConfig({
    name: 'Grok',
    host_field: '',
    allow_empty_host: true,
    required_params: ['match'],
    optional_params: ['extra_patterns_file', 'tag_on_failure'],
    default_values: {
      'tag_on_failure': ['_grokparsefailure']
    },
    arrays: ['tag_on_failure'],
...
FilterGrok.prototype.start = function(callback) {
  logger.info('Initializing grok filter, pattern: ' + this.match);
  ...
  var _done = function(extra_patterns) {
    try {
      var expanded = this.expandGrokPattern(this.match, extra_patterns);
      this.regex = new OnigRegExp(expanded);
    }
    catch(e) {
      logger.error('Unable to process grok pattern', this.match, e);
      return callback(e);
    }
    callback();
  }.bind(this);
...
FilterGrok.prototype.process = function(data) {
  logger.debug('Trying to match on grok', this.match, ', input', data.message);
  var result = this.regex.searchSync(data.message);
  if (result) {
    for (var i = 0; i < this.fields.length; i++) {
      this.post_process(data, result[i + 1].match, i);
    }
    this.add_tags_function(data, 'add_tags');
    this.remove_tags_function(data);
    this.add_fields_function(data);
    this.remove_fields_function(data);
  }
  else {
    this.add_tags_function(data, 'tag_on_failure');
  }
  return data;
};
