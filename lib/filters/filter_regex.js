...
function FilterRegex() {
  ...
  this.mergeConfig({
    name: 'Regex',
    host_field: 'builtin_regex',
    allow_empty_host: true,
    required_params: ['regex'],
    optional_params: ['fields', 'regex_flags', 'field'],
    default_values: {
      'field': 'message',
      'fields': '',
      'numerical_fields': '',
...
FilterRegex.prototype.loadPattern = function(callback) {
  if (this.builtin_regex) {
    logger.info('Try to load config from pattern file ' + this.builtin_regex);
    patterns_loader.load(this.builtin_regex, function(err, config) {
      if (err) {
        return callback(new Error('Unable to load pattern : ' + this.builtin_regex + ' : ' + err));
      }
      for (var i in config) {
        this[i] = config[i];
      }
...
FilterRegex.prototype.start = function(callback) {
  ...
  this.fields = Array.isArray(this.fields) ? this.fields : this.fields.split(',');
...
FilterRegex.prototype.process = function(data) {
  logger.debug('Trying to match on regex', this.regex, ', input', data[this.field]);
  if (data[this.field]) {
    var result = data[this.field].match(this.regex);
