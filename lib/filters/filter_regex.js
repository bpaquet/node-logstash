...
FilterRegex.prototype.loadPattern = function(callback) {
  if (this.pattern) {
    logger.info('Try to load config from pattern file ' + this.pattern);
    patterns_loader.load(this.pattern, function(err, config) {
      if (err) {
        return callback(new Error('Unable to load pattern : ' + this.pattern + ' : ' + err));
      }
      for (var i in config) {
        this[i] = config[i];
      }
