...
FilterRegex.prototype.process = function(data) {
  logger.debug('Trying to match on regex', this.regex, ', input', data.getMessage());
  var result = data.getMessage().match(this.regex);
    ...
    for(var i = 0; i < this.fields.length; i ++) {
      ...
      if (v) {
        if (this.date_format && this.fields[i] == 'timestamp') {
          data.setTimestamp(moment(v, this.date_format).format());
          logger.debug('Event timestamp modified to', data.getTimestamp());
        }
        else {
          if (v.match(/^[0-9]+$/)) {
            v = parseInt(v);
