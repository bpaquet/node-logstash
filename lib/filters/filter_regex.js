...
function FilterRegex() {
  ...
  this.config = {
    ...
    optional_params: ['regex', 'fields', 'date_format', 'output_date_format']
  }
}
...
FilterRegex.prototype.afterLoadConfig = function(callback) {
  ...
  logger.info('Initializing regex filter, regex : ' + this.regex + ', fields ' + this.fields
             + (this.date_format ? ', date format ' + this.date_format : '')
             + (this.output_date_format ? ', output date format ' + this.output_date_format : ''));
      ...
      if (v) {
        if (this.date_format && this.fields[i] == 'timestamp') {
          ...
          if (m.year() + m.month() + m.date() + m.hours() + m.minutes() + m.seconds() > 1) {
            ...
            data['@timestamp'] = m.format(this.output_date_format);
