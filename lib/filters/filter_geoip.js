...
FilterGeoip.prototype.process = function(data) {
  if (data[this.use_attr]) {
    try {
      ...
      if (this.save_country) { data[this.use_attr + '_geo_country'] = geo['country'];}
      if (this.save_region) { data[this.use_attr + '_geo_region'] = geo['region'];}
      if (this.save_city) { data[this.use_attr + '_geo_city'] = geo['city'];}
      if (this.save_latlon) { data[this.use_attr + '_geo_lat'] = geo['ll'][0];
                              data[this.use_attr + '_geo_lon'] = geo['ll'][1];
                            }
    }
    catch(err) {
      return data;
