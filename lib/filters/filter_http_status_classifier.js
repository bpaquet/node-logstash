...
FilterHttpStatusClassifier.prototype.start = function(callback) {
  ...
  logger.info('Initialized http status filter: source ' + this.field + ', target: ' + this.target_field + ', special_codes: ' + this.special_codes);
  callback();
};
...
FilterHttpStatusClassifier.prototype.process = function(data) {
  if (data[this.field]) {
    for(var i in this.special_codes) {
      if (data[this.field] === this.special_codes[i]) {
        data[this.target_field] = this.special_codes[i].toString();
        return data;
      }
    }
    if (data[this.field] >= 100 && data[this.field] < 200) {
      data[this.target_field] = '1xx';
    }
    if (data[this.field] >= 200 && data[this.field] < 300) {
      data[this.target_field] = '2xx';
    }
    if (data[this.field] >= 300 && data[this.field] < 400) {
      data[this.target_field] = '3xx';
    }
    if (data[this.field] >= 400 && data[this.field] < 500) {
      data[this.target_field] = '4xx';
    }
    if (data[this.field] >= 500 && data[this.field] < 600) {
