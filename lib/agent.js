...
function LogstashAgent() {
  ...
  this.sig_listener = function() {
    if (this.closed_inputs) {
      ...
      this.start_inputs(function(err) {
        if (err) {
          logger.err('Unable to start plugins', err);
        }
      });
    }
    else {
      logger.info('Closing inputs plugins');
      this.close_inputs(function(err) {
        if (err) {
          logger.err('Unable to close plugins', err);
        }
      });
    }
  }.bind(this);
...
LogstashAgent.prototype.close_inputs = function(callback) {
  this.close_modules(this.modules.inputs.slice(0), function(err) {
    if (err) {
      return callback(err);
    }
...
LogstashAgent.prototype.start_inputs = function(callback) {
  this.start_modules(this.modules.inputs.slice(0), function(err) {
    if (err) {
      return callback(err);
    }
